resource "ultradns_record" "cname-editorials-watch-aetnd-com" {
  zone_name   = "aetnd.com"
  record_type = "CNAME"
  owner_name  = "editorials.watch.aetnd.com."
  record_data = [
...
resource "pagerduty_escalation_policy" "prod-watch-editorials-escalation-policy" {
  name      = "prod-watch-editorials-escalation-policy"
  num_loops = 3
  rule {
...
resource "pagerduty_service" "prod-watch-editorials-service" {
  name              = "prod-watch-editorials-service"
  escalation_policy = pagerduty_escalation_policy.prod-watch-editorials-escalation-policy.id
...
resource "pagerduty_service_integration" "prod-watch-editorials-integration-cloudwatch" {
  name    = data.pagerduty_vendor.cloudwatch.name
  vendor  = data.pagerduty_vendor.cloudwatch.id
  service = pagerduty_service.prod-watch-editorials-service.id
}

resource "pagerduty_service_integration" "prod-watch-editorials-integration-datadog" {
  name    = pagerduty_service.prod-watch-editorials-service.name
  service = pagerduty_service.prod-watch-editorials-service.id
  vendor  = data.pagerduty_vendor.datadog.id
}

resource "datadog_integration_pagerduty_service_object" "prod-watch-editorials-service" {
  ...
  service_name = pagerduty_service.prod-watch-editorials-service.name
  service_key  = pagerduty_service_integration.prod-watch-editorials-integration-datadog.integration_key
}

resource "aws_cloudformation_stack" "prod-watch-editorials-alert" {
  provider          = aws.us-east-1
  name              = "prod-watch-editorials-alert"
  ...
  parameters = {
    Email         = null
    HttpEndpoint  = null
    HttpsEndpoint = "https://events.pagerduty.com/integration/${pagerduty_service_integration.prod-watch-editorials-integration-cloudwatch.integration_key}/enqueue"
...
resource "aws_cloudformation_stack" "prod-watch-editorials-iam" {
  provider          = aws.us-east-1
  name              = "prod-watch-editorials-iam"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
...
resource "aws_cloudformation_stack" "prod-watch-editorials-pipeline" {
  provider          = aws.us-east-1
  name              = "prod-watch-editorials-pipeline"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
  ...
  parameters = {
    ...
    CodeBuildServicePolicies    = aws_cloudformation_stack.prod-watch-editorials-iam.outputs.PolicyArn
