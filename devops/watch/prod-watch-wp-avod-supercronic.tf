resource "pagerduty_escalation_policy" "prod-watch-wp-avod-supercronic-escalation-policy" {
  name      = "prod-watch-wp-avod-supercronic-escalation-policy"
  num_loops = 3
  rule {
...
resource "pagerduty_service" "prod-watch-wp-avod-supercronic-service" {
  name              = "prod-watch-wp-avod-supercronic-service"
  escalation_policy = pagerduty_escalation_policy.prod-watch-wp-avod-supercronic-escalation-policy.id
...
resource "pagerduty_service_integration" "prod-watch-wp-avod-supercronic-integration-cloudwatch" {
  name    = data.pagerduty_vendor.cloudwatch.name
  vendor  = data.pagerduty_vendor.cloudwatch.id
  service = pagerduty_service.prod-watch-wp-avod-supercronic-service.id
}

resource "pagerduty_service_integration" "prod-watch-wp-avod-supercronic-integration-datadog" {
  name    = pagerduty_service.prod-watch-wp-avod-supercronic-service.name
  service = pagerduty_service.prod-watch-wp-avod-supercronic-service.id
  vendor  = data.pagerduty_vendor.datadog.id
}

resource "datadog_integration_pagerduty_service_object" "prod-watch-wp-avod-supercronic-service" {
  ...
  service_name = pagerduty_service.prod-watch-wp-avod-supercronic-service.name
  service_key  = pagerduty_service_integration.prod-watch-wp-avod-supercronic-integration-datadog.integration_key
}

resource "aws_cloudformation_stack" "prod-watch-wp-avod-supercronic-alert" {
  provider          = aws.us-east-1
  name              = "prod-watch-wp-avod-supercronic-alert"
  ...
  parameters = {
    Email         = null
    HttpEndpoint  = null
    HttpsEndpoint = "https://events.pagerduty.com/integration/${pagerduty_service_integration.prod-watch-wp-avod-supercronic-integration-cloudwatch.integration_key}/enqueue"
...
resource "aws_cloudformation_stack" "prod-watch-wp-avod-supercronic-ecr" {
  provider          = aws.us-east-1
  name              = "prod-watch-wp-avod-supercronic-ecr"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
  ...
  tags = {
    ...
    "Application ID"             = "WPW"
    Role                         = "infra"
    "aetnd:application"          = "watch-wp-avod-supercronic"
    "aetnd:env"                  = "prod"
    "Line of Business - Primary" = "digital"
    env                          = "prod"
    Environment                  = "prod"
...
resource "aws_cloudformation_stack" "prod-watch-wp-avod-supercronic-service" {
  provider          = aws.us-east-1
  name              = "prod-watch-wp-avod-supercronic-service"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  ...
  parameters = {
    ...
    ParentECRRepositoryStack                       = aws_cloudformation_stack.prod-watch-wp-avod-supercronic-ecr.outputs.StackName
    ParentAlertStack                               = aws_cloudformation_stack.prod-watch-wp-avod-supercronic-alert.outputs.StackName
    ...
    AppCommand                                     = "docker-avod-supercronic-entrypoint"
    AppUser                                        = null
    AppPort                                        = "8080"
...
resource "aws_cloudformation_stack" "prod-watch-wp-avod-supercronic-pipeline" {
  provider          = aws.us-east-1
  name              = "prod-watch-wp-avod-supercronic-pipeline"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
  ...
  parameters = {
    ParentClusterStack          = "prod-watch-cluster"
    ParentServiceStack          = aws_cloudformation_stack.prod-watch-wp-avod-supercronic-service.outputs.StackName
    ParentECRRepositoryStack    = aws_cloudformation_stack.prod-watch-wp-avod-supercronic-ecr.outputs.StackName
