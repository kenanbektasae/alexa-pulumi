resource "pagerduty_escalation_policy" "prod-watch-alexa-supercronic-escalation-policy" {
  name      = "prod-watch-alexa-supercronic-escalation-policy"
  num_loops = 3
  rule {
...
resource "pagerduty_service" "prod-watch-alexa-supercronic-service" {
  name              = "prod-watch-alexa-supercronic-service"
  escalation_policy = pagerduty_escalation_policy.prod-watch-alexa-supercronic-escalation-policy.id
...
resource "pagerduty_service_integration" "prod-watch-alexa-supercronic-integration-cloudwatch" {
  name    = data.pagerduty_vendor.cloudwatch.name
  vendor  = data.pagerduty_vendor.cloudwatch.id
  service = pagerduty_service.prod-watch-alexa-supercronic-service.id
}

resource "pagerduty_service_integration" "prod-watch-alexa-supercronic-integration-datadog" {
  name    = pagerduty_service.prod-watch-alexa-supercronic-service.name
  service = pagerduty_service.prod-watch-alexa-supercronic-service.id
  vendor  = data.pagerduty_vendor.datadog.id
}

resource "datadog_integration_pagerduty_service_object" "prod-watch-alexa-supercronic-service" {
  ...
  service_name = pagerduty_service.prod-watch-alexa-supercronic-service.name
  service_key  = pagerduty_service_integration.prod-watch-alexa-supercronic-integration-datadog.integration_key
}

resource "aws_cloudformation_stack" "prod-watch-alexa-supercronic-alert" {
  provider          = aws.us-east-1
  name              = "prod-watch-alexa-supercronic-alert"
  ...
  parameters = {
    Email         = null
    HttpEndpoint  = null
    HttpsEndpoint = "https://events.pagerduty.com/integration/${pagerduty_service_integration.prod-watch-alexa-supercronic-integration-cloudwatch.integration_key}/enqueue"
...
resource "aws_cloudformation_stack" "prod-watch-alexa-supercronic-ecr" {
  provider          = aws.us-east-1
  name              = "prod-watch-alexa-supercronic-ecr"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
...
resource "aws_cloudformation_stack" "prod-watch-alexa-supercronic-service" {
  provider          = aws.us-east-1
  name              = "prod-watch-alexa-supercronic-service"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  ...
  parameters = {
    ...
    ParentECRRepositoryStack                       = aws_cloudformation_stack.prod-watch-alexa-supercronic-ecr.outputs.StackName
    ParentAlertStack                               = aws_cloudformation_stack.prod-watch-alexa-supercronic-alert.outputs.StackName
...
resource "aws_cloudformation_stack" "prod-watch-alexa-supercronic-pipeline" {
  provider          = aws.us-east-1
  name              = "prod-watch-alexa-supercronic-pipeline"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
  ...
  parameters = {
    ParentClusterStack          = "prod-watch-cluster"
    ParentServiceStack          = aws_cloudformation_stack.prod-watch-alexa-supercronic-service.outputs.StackName
    ParentECRRepositoryStack    = aws_cloudformation_stack.prod-watch-alexa-supercronic-ecr.outputs.StackName
