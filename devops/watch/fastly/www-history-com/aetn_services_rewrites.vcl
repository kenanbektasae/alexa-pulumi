sub aetn_services_rewrites {
    ...
    if (req.http.host == if(req.http.AETN-env, std.tolower(req.http.AETN-env)"-historyenespanol.com", "historyenespanol.com")) {
      set req.http.host = if(req.http.AETN-env, std.tolower(req.http.AETN-env)".history.com", "www.history.com");
      set req.url = "/espanol/schedule";
      error 701 req.url;
    }
    ...
    if (req.http.AETN-env) {
      if (req.http.host == regsub(std.tolower(req.http.AETN-env), "(.*)", "\1.historyenespanol.com")) {
        set req.http.host = "www.history.com";
        set req.url = "/";
        error 701 req.url;
      }
