module "dev-cropper-watch-aetnd-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "dev-cropper.watch.aetnd.com"

  domains = [
    "dev-cropper.watch.aetnd.com"
  ]
  ...
  backends = {
    TLS_Backend = {
      address     = "dev-watch-ingress.aetnd.io"
      healthcheck = "cropper-HC"
    }
  }

  healthchecks = {
    "cropper-HC" = {
      host = "dev-cropper.watch.aetnd.com"
    }
  }
  ...
  snippets = {
    ...
    aetn_services_recv         = { type = "init", priority = 100, content = file("${path.module}/fastly/cropper-watch-aetnd-com/aetn_services_recv.vcl") }
    aetn_default_backend       = { type = "init", priority = 100, content = file("${path.module}/fastly/cropper-watch-aetnd-com/aetn_default_backend.vcl") }
    aetn_services_rewrites     = { type = "init", priority = 100, content = file("${path.module}/fastly/cropper-watch-aetnd-com/aetn_services_rewrites.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/cropper-watch-aetnd-com/default_sigsci.vcl")
    }
  }
  ...
  sigsci_backends  = ["F_TLS_Backend"]
  tags = {
    "aetnd:service_name" = "dev-watch-cropper"
  }
}
