module "prod-s3-mylifetime-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "s3.mylifetime.com"
  ...
  snippets = {
    ...
    aetn_services_proxypass    = { type = "init", priority = 100, content = file("${path.module}/fastly/s3-mylifetime-com/aetn_services_proxypass.vcl") }
    aetn_default_backend       = { type = "init", priority = 100, content = file("${path.module}/fastly/s3-mylifetime-com/aetn_default_backend.vcl") }
    aetn_service_cache_fetch   = { type = "init", priority = 100, content = file("${path.module}/fastly/s3-mylifetime-com/aetn_service_cache_fetch.vcl") }
    aetn_services_recv         = { type = "init", priority = 100, content = file("${path.module}/fastly/s3-mylifetime-com/aetn_services_recv.vcl") }
    backend                    = { type = "init", priority = 100, content = file("${path.module}/fastly/s3-mylifetime-com/prod_backend.vcl") }
    redirects                  = { type = "init", priority = 100, content = file("${path.module}/fastly/s3-mylifetime-com/redirects.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/s3-mylifetime-com/default.vcl")
    }
  }
