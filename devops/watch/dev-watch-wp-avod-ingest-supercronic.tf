resource "pagerduty_escalation_policy" "dev-watch-wp-avod-ingest-supercronic-escalation-policy" {
  name      = "dev-watch-wp-avod-ingest-supercronic-escalation-policy"
  num_loops = 3
  rule {
...
resource "pagerduty_service" "dev-watch-wp-avod-ingest-supercronic-service" {
  name              = "dev-watch-wp-avod-ingest-supercronic-service"
  escalation_policy = pagerduty_escalation_policy.dev-watch-wp-avod-ingest-supercronic-escalation-policy.id
...
resource "pagerduty_service_integration" "dev-watch-wp-avod-ingest-supercronic-integration-cloudwatch" {
  name    = data.pagerduty_vendor.cloudwatch.name
  vendor  = data.pagerduty_vendor.cloudwatch.id
  service = pagerduty_service.dev-watch-wp-avod-ingest-supercronic-service.id
}

resource "pagerduty_service_integration" "dev-watch-wp-avod-ingest-supercronic-integration-datadog" {
  name    = pagerduty_service.dev-watch-wp-avod-ingest-supercronic-service.name
  service = pagerduty_service.dev-watch-wp-avod-ingest-supercronic-service.id
  vendor  = data.pagerduty_vendor.datadog.id
}

resource "datadog_integration_pagerduty_service_object" "dev-watch-wp-avod-ingest-supercronic-service" {
  ...
  service_name = pagerduty_service.dev-watch-wp-avod-ingest-supercronic-service.name
  service_key  = pagerduty_service_integration.dev-watch-wp-avod-ingest-supercronic-integration-datadog.integration_key
}

resource "aws_cloudformation_stack" "dev-watch-wp-avod-ingest-supercronic-alert" {
  provider          = aws.us-east-1
  name              = "dev-watch-wp-avod-ingest-supercronic-alert"
  ...
  parameters = {
    Email         = null
    HttpEndpoint  = null
    HttpsEndpoint = "https://events.pagerduty.com/integration/${pagerduty_service_integration.dev-watch-wp-avod-ingest-supercronic-integration-cloudwatch.integration_key}/enqueue"
...
resource "aws_cloudformation_stack" "dev-watch-wp-avod-ingest-supercronic-ecr" {
  provider          = aws.us-east-1
  name              = "dev-watch-wp-avod-ingest-supercronic-ecr"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
...
resource "aws_cloudformation_stack" "dev-watch-wp-avod-ingest-supercronic-service" {
  provider          = aws.us-east-1
  name              = "dev-watch-wp-avod-ingest-supercronic-service"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  ...
  parameters = {
    ...
    ParentECRRepositoryStack                       = aws_cloudformation_stack.dev-watch-wp-avod-ingest-supercronic-ecr.outputs.StackName
    ParentAlertStack                               = aws_cloudformation_stack.dev-watch-wp-avod-ingest-supercronic-alert.outputs.StackName
...
resource "aws_cloudformation_stack" "dev-watch-wp-avod-ingest-supercronic-pipeline" {
  provider          = aws.us-east-1
  name              = "dev-watch-wp-avod-ingest-supercronic-pipeline"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
  ...
  parameters = {
    ParentClusterStack          = "dev-watch-cluster"
    ParentServiceStack          = aws_cloudformation_stack.dev-watch-wp-avod-ingest-supercronic-service.outputs.StackName
    ParentECRRepositoryStack    = aws_cloudformation_stack.dev-watch-wp-avod-ingest-supercronic-ecr.outputs.StackName
