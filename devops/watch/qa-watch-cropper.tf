resource "ultradns_record" "cname-qa-cropper-watch-aetnd-com" {
  zone_name   = "aetnd.com"
  record_type = "CNAME"
  owner_name  = "qa-cropper.watch.aetnd.com."
  record_data = [
...
resource "pagerduty_escalation_policy" "qa-watch-cropper-escalation-policy" {
  name      = "qa-watch-cropper-escalation-policy"
  num_loops = 3
  rule {
...
resource "pagerduty_service" "qa-watch-cropper-service" {
  name              = "qa-watch-cropper-service"
  escalation_policy = pagerduty_escalation_policy.qa-watch-cropper-escalation-policy.id
...
resource "pagerduty_service_integration" "qa-watch-cropper-integration-cloudwatch" {
  name    = data.pagerduty_vendor.cloudwatch.name
  vendor  = data.pagerduty_vendor.cloudwatch.id
  service = pagerduty_service.qa-watch-cropper-service.id
}

resource "pagerduty_service_integration" "qa-watch-cropper-integration-datadog" {
  name    = pagerduty_service.qa-watch-cropper-service.name
  service = pagerduty_service.qa-watch-cropper-service.id
  vendor  = data.pagerduty_vendor.datadog.id
}

resource "datadog_integration_pagerduty_service_object" "qa-watch-cropper-service" {
  ...
  service_name = pagerduty_service.qa-watch-cropper-service.name
  service_key  = pagerduty_service_integration.qa-watch-cropper-integration-datadog.integration_key
}

resource "aws_cloudformation_stack" "qa-watch-cropper-alert" {
  provider          = aws.us-east-1
  name              = "qa-watch-cropper-alert"
  ...
  parameters = {
    Email         = null
    HttpEndpoint  = null
    HttpsEndpoint = "https://events.pagerduty.com/integration/${pagerduty_service_integration.qa-watch-cropper-integration-cloudwatch.integration_key}/enqueue"
...
resource "aws_cloudformation_stack" "qa-watch-cropper-ecr" {
  provider          = aws.us-east-1
  name              = "qa-watch-cropper-ecr"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
...
resource "aws_cloudformation_stack" "qa-watch-cropper-sg" {
  provider          = aws.us-east-1
  name              = "qa-watch-cropper-sg"
  capabilities      = []
  disable_rollback  = false
  notification_arns = []
...
resource "aws_cloudformation_stack" "qa-watch-cropper-iam" {
  provider          = aws.us-east-1
  name              = "qa-watch-cropper-iam"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
...
resource "aws_cloudformation_stack" "qa-watch-cropper-service" {
  provider          = aws.us-east-1
  name              = "qa-watch-cropper-service"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  ...
  parameters = {
    ...
    ParentECRRepositoryStack                                = aws_cloudformation_stack.qa-watch-cropper-ecr.outputs.StackName
    ParentAlertStack                                        = aws_cloudformation_stack.qa-watch-cropper-alert.outputs.StackName
    ParentZoneStack                                         = null
    ParentClientStack1                                      = aws_cloudformation_stack.qa-watch-cropper-sg.outputs.StackName
    ParentClientStack2                                      = "sg-qa"
    ParentClientStack3                                      = null
    LoadBalancerPriority                                    = "10"
    LoadBalancerHostPattern                                 = "qa-cropper.watch.aetnd.com"
    LoadBalancerPathPattern                                 = "/*"
    LoadBalancerDeregistrationDelay                         = "60"
    LoadBalancerStickiness                                  = "false"
    TaskPolicies                                            = aws_cloudformation_stack.qa-watch-cropper-iam.outputs.PolicyArn
    ProxyImage                                              = null
    ProxyCommand                                            = null
    ProxyPort                                               = "8000"
...
resource "aws_cloudformation_stack" "qa-watch-cropper-pipeline" {
  provider          = aws.us-east-1
  name              = "qa-watch-cropper-pipeline"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
  ...
  parameters = {
    ParentClusterStack          = "qa-watch-cluster"
    ParentServiceStack          = aws_cloudformation_stack.qa-watch-cropper-service.outputs.StackName
    ParentECRRepositoryStack    = aws_cloudformation_stack.qa-watch-cropper-ecr.outputs.StackName
