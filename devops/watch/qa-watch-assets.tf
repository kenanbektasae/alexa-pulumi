resource "ultradns_record" "cname-qa-assets-watch-aetnd-com" {
  zone_name   = "aetnd.com"
  record_type = "CNAME"
  owner_name  = "qa-assets.watch.aetnd.com."
  record_data = [
...
resource "pagerduty_escalation_policy" "qa-watch-assets-escalation-policy" {
  name      = "qa-watch-assets-escalation-policy"
  num_loops = 3
  rule {
...
resource "pagerduty_service" "qa-watch-assets-service" {
  name              = "qa-watch-assets-service"
  escalation_policy = pagerduty_escalation_policy.qa-watch-assets-escalation-policy.id
...
resource "pagerduty_service_integration" "qa-watch-assets-integration-cloudwatch" {
  name    = data.pagerduty_vendor.cloudwatch.name
  vendor  = data.pagerduty_vendor.cloudwatch.id
  service = pagerduty_service.qa-watch-assets-service.id
}

resource "pagerduty_service_integration" "qa-watch-assets-integration-datadog" {
  name    = pagerduty_service.qa-watch-assets-service.name
  service = pagerduty_service.qa-watch-assets-service.id
  vendor  = data.pagerduty_vendor.datadog.id
}

resource "datadog_integration_pagerduty_service_object" "qa-watch-assets-service" {
  ...
  service_name = pagerduty_service.qa-watch-assets-service.name
  service_key  = pagerduty_service_integration.qa-watch-assets-integration-datadog.integration_key
}

resource "aws_cloudformation_stack" "qa-watch-assets-alert" {
  provider          = aws.us-east-1
  name              = "qa-watch-assets-alert"
  ...
  parameters = {
    Email         = null
    HttpEndpoint  = null
    HttpsEndpoint = "https://events.pagerduty.com/integration/${pagerduty_service_integration.qa-watch-assets-integration-cloudwatch.integration_key}/enqueue"
...
resource "aws_cloudformation_stack" "qa-watch-assets-iam" {
  provider          = aws.us-east-1
  name              = "qa-watch-assets-iam"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
...
resource "aws_cloudformation_stack" "qa-watch-assets-pipeline" {
  provider          = aws.us-east-1
  name              = "qa-watch-assets-pipeline"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
  ...
  parameters = {
    ...
    CodeBuildServicePolicies    = aws_cloudformation_stack.qa-watch-assets-iam.outputs.PolicyArn
