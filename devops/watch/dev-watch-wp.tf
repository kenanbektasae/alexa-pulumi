resource "ultradns_record" "cname-dev-wp-watch-aetnd-com" {
  zone_name   = "aetnd.com"
  record_type = "CNAME"
  owner_name  = "dev-wp.watch.aetnd.com."
  record_data = [
...
resource "pagerduty_escalation_policy" "dev-watch-wp-escalation-policy" {
  name      = "dev-watch-wp-escalation-policy"
  num_loops = 3
  rule {
...
resource "pagerduty_service" "dev-watch-wp-service" {
  name              = "dev-watch-wp-service"
  escalation_policy = pagerduty_escalation_policy.dev-watch-wp-escalation-policy.id
...
resource "pagerduty_service_integration" "dev-watch-wp-integration-cloudwatch" {
  name    = data.pagerduty_vendor.cloudwatch.name
  vendor  = data.pagerduty_vendor.cloudwatch.id
  service = pagerduty_service.dev-watch-wp-service.id
}

resource "pagerduty_service_integration" "dev-watch-wp-integration-datadog" {
  name    = pagerduty_service.dev-watch-wp-service.name
  service = pagerduty_service.dev-watch-wp-service.id
  vendor  = data.pagerduty_vendor.datadog.id
}

resource "datadog_integration_pagerduty_service_object" "dev-watch-wp-service" {
  ...
  service_name = pagerduty_service.dev-watch-wp-service.name
  service_key  = pagerduty_service_integration.dev-watch-wp-integration-datadog.integration_key
}

resource "aws_cloudformation_stack" "dev-watch-wp-alert" {
  provider          = aws.us-east-1
  name              = "dev-watch-wp-alert"
  ...
  parameters = {
    Email         = null
    HttpEndpoint  = null
    HttpsEndpoint = "https://events.pagerduty.com/integration/${pagerduty_service_integration.dev-watch-wp-integration-cloudwatch.integration_key}/enqueue"
...
resource "aws_cloudformation_stack" "dev-watch-wp-ecr" {
  provider          = aws.us-east-1
  name              = "dev-watch-wp-ecr"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
...
resource "aws_cloudformation_stack" "dev-watch-wp-sg" {
  provider          = aws.us-east-1
  name              = "dev-watch-wp-sg"
  capabilities      = []
  disable_rollback  = false
  notification_arns = []
...
resource "aws_cloudformation_stack" "dev-watch-wp-iam" {
  provider          = aws.us-east-1
  name              = "dev-watch-wp-iam"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
...
resource "aws_cloudformation_stack" "dev-watch-wp-sqs" {
  provider          = aws.us-east-1
  name              = "dev-watch-wp-sqs"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
  template_body     = file("${path.module}/templates/sqs.yaml")
  parameters = {
    ParentAlertStack                                 = aws_cloudformation_stack.dev-watch-wp-alert.outputs.StackName
    ParentKmsKeyStack                                = null
    QueueName                                        = null
    FifoQueue                                        = "standard"
...
resource "aws_cloudformation_stack" "dev-watch-wp-service" {
  provider          = aws.us-east-1
  name              = "dev-watch-wp-service"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  ...
  parameters = {
    ...
    ParentECRRepositoryStack                                = aws_cloudformation_stack.dev-watch-wp-ecr.outputs.StackName
    ParentAlertStack                                        = aws_cloudformation_stack.dev-watch-wp-alert.outputs.StackName
    ...
    LoadBalancerHostPattern                                 = "dev-wp.watch.aetnd.com"
...
resource "aws_cloudformation_stack" "dev-watch-wp-pipeline" {
  provider          = aws.us-east-1
  name              = "dev-watch-wp-pipeline"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
  ...
  parameters = {
    ParentClusterStack          = "dev-watch-cluster"
    ParentServiceStack          = aws_cloudformation_stack.dev-watch-wp-service.outputs.StackName
    ParentECRRepositoryStack    = aws_cloudformation_stack.dev-watch-wp-ecr.outputs.StackName
