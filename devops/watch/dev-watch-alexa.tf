resource "ultradns_record" "cname-dev-alexa-watch-aetnd-com" {
  zone_name   = "aetnd.com"
  record_type = "CNAME"
  owner_name  = "dev-alexa.watch.aetnd.com."
  record_data = [
...
resource "pagerduty_escalation_policy" "dev-watch-alexa-escalation-policy" {
...
resource "pagerduty_service" "dev-watch-alexa-service" {
  name              = "dev-watch-alexa-service"
  escalation_policy = pagerduty_escalation_policy.dev-watch-alexa-escalation-policy.id
...
resource "pagerduty_service_integration" "dev-watch-alexa-integration-cloudwatch" {
  name    = data.pagerduty_vendor.cloudwatch.name
  vendor  = data.pagerduty_vendor.cloudwatch.id
  service = pagerduty_service.dev-watch-alexa-service.id
}

resource "pagerduty_service_integration" "dev-watch-alexa-integration-datadog" {
  name    = pagerduty_service.dev-watch-alexa-service.name
  service = pagerduty_service.dev-watch-alexa-service.id
  vendor  = data.pagerduty_vendor.datadog.id
}

resource "datadog_integration_pagerduty_service_object" "dev-watch-alexa-service" {
  ...
  service_name = pagerduty_service.dev-watch-alexa-service.name
  service_key  = pagerduty_service_integration.dev-watch-alexa-integration-datadog.integration_key
}

resource "aws_cloudformation_stack" "dev-watch-alexa-alert" {
  ...
  parameters = {
    Email         = null
    HttpEndpoint  = null
    HttpsEndpoint = "https://events.pagerduty.com/integration/${pagerduty_service_integration.dev-watch-alexa-integration-cloudwatch.integration_key}/enqueue"
...
resource "aws_cloudformation_stack" "dev-watch-alexa-ecr" {
  provider          = aws.us-east-1
...
resource "aws_cloudformation_stack" "dev-watch-alexa-sg" {
  provider          = aws.us-east-1
...
resource "aws_cloudformation_stack" "dev-watch-alexa-iam" {
  provider          = aws.us-east-1
...
resource "aws_cloudformation_stack" "dev-watch-alexa-cache-1" {
  provider          = aws.us-east-1
  ...
  parameters = {
    ...
    ParentAlertStack                     = aws_cloudformation_stack.dev-watch-alexa-alert.outputs.StackName
...
resource "aws_cloudformation_stack" "dev-watch-alexa-service" {
  provider          = aws.us-east-1
  name              = "dev-watch-alexa-service"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  ...
  parameters = {
    ...
    ParentECRRepositoryStack                                = aws_cloudformation_stack.dev-watch-alexa-ecr.outputs.StackName
    ParentAlertStack                                        = aws_cloudformation_stack.dev-watch-alexa-alert.outputs.StackName
...
resource "aws_cloudformation_stack" "dev-watch-alexa-pipeline" {
  provider          = aws.us-east-1
  ...
  parameters = {
    ParentClusterStack          = "dev-watch-cluster"
    ParentServiceStack          = aws_cloudformation_stack.dev-watch-alexa-service.outputs.StackName
    ParentECRRepositoryStack    = aws_cloudformation_stack.dev-watch-alexa-ecr.outputs.StackName
