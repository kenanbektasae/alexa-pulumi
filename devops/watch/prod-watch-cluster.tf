resource "pagerduty_escalation_policy" "prod-watch-cluster-escalation-policy" {
  name      = "prod-watch-cluster-escalation-policy"
  num_loops = 3
  rule {
...
resource "pagerduty_service" "prod-watch-cluster-service" {
  name              = "prod-watch-cluster-service"
  escalation_policy = pagerduty_escalation_policy.prod-watch-cluster-escalation-policy.id
...
resource "pagerduty_service_integration" "prod-watch-cluster-integration-cloudwatch" {
  name    = data.pagerduty_vendor.cloudwatch.name
  vendor  = data.pagerduty_vendor.cloudwatch.id
  service = pagerduty_service.prod-watch-cluster-service.id
}

resource "pagerduty_service_integration" "prod-watch-cluster-integration-datadog" {
  name    = pagerduty_service.prod-watch-cluster-service.name
  service = pagerduty_service.prod-watch-cluster-service.id
  vendor  = data.pagerduty_vendor.datadog.id
}

resource "datadog_integration_pagerduty_service_object" "prod-watch-cluster-service" {
  ...
  service_name = pagerduty_service.prod-watch-cluster-service.name
  service_key  = pagerduty_service_integration.prod-watch-cluster-integration-datadog.integration_key
}

resource "aws_cloudformation_stack" "prod-watch-cluster-alert" {
  provider          = aws.us-east-1
  name              = "prod-watch-cluster-alert"
  capabilities      = []
  ...
  parameters = {
    Email         = null
    HttpEndpoint  = null
    HttpsEndpoint = "https://events.pagerduty.com/integration/${pagerduty_service_integration.prod-watch-cluster-integration-cloudwatch.integration_key}/enqueue"
...
resource "aws_cloudformation_stack" "prod-watch-cluster" {
  provider          = aws.us-east-1
  name              = "prod-watch-cluster"
  ...
  parameters = {
    ...
    ParentAlertStack                                     = aws_cloudformation_stack.prod-watch-cluster-alert.outputs.StackName
    ...
    SubDomainNameWithDot                                 = "prod-watch-ingress."
    DNSRecordWeight                                      = "50"
