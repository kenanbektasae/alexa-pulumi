module "prod-maintenance-history-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "maintenance.history.com"

  domains = [
    "maintenance.history.com",
    "maintenance.aetv.com",
    "maintenance.crimeandinvestigationnetwork.com",
    "maintenance.fyi.tv",
    "maintenance.historyvault.com",
    "maintenance.lifetimemovieclub.com",
    "maintenance.mylifetime.com"
  ]
  ...
  snippets = {
    ...
    aetn_default_backend       = { type = "init", priority = 100, content = file("${path.module}/fastly/maintenance-history-com/aetn_default_backend.vcl") }
    aetn_service_cache_fetch   = { type = "init", priority = 100, content = file("${path.module}/fastly/maintenance-history-com/aetn_service_cache_fetch.vcl") }
    aetn_services_recv         = { type = "init", priority = 100, content = file("${path.module}/fastly/maintenance-history-com/aetn_services_recv.vcl") }
    aetn_services_rewrites     = { type = "init", priority = 100, content = file("${path.module}/fastly/maintenance-history-com/aetn_services_rewrites.vcl") }
    backend                    = { type = "init", priority = 100, content = file("${path.module}/fastly/maintenance-history-com/prod_backend.vcl") }
    redirects                  = { type = "init", priority = 100, content = file("${path.module}/fastly/maintenance-history-com/redirects.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/maintenance-history-com/default.vcl")
    }
  }
