module "dev-assets-watch-aetnd-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "dev-assets.watch.aetnd.com"

  domains = [
    "dev-assets.watch.aetnd.com"
  ]
  ...
  snippets = {
    ...
    aetn_services_recv                 = { type = "init", priority = 100, content = file("${path.module}/fastly/assets-watch-aetnd-com/aetn_services_recv.vcl") }
    aetn_default_backend               = { type = "init", priority = 100, content = file("${path.module}/fastly/assets-watch-aetnd-com/aetn_default_backend.vcl") }
    aetn_services_rewrites             = { type = "init", priority = 100, content = file("${path.module}/fastly/assets-watch-aetnd-com/aetn_services_rewrites.vcl") }
    redirects                          = { type = "init", priority = 100, content = file("${path.module}/fastly/assets-watch-aetnd-com/redirects.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/assets-watch-aetnd-com/default.vcl")
    }
  }
