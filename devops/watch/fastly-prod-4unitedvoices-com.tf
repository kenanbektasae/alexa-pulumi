module "prod-4unitedvoices-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "4unitedvoices.com"

  domains = [
    "4unitedvoices.com",
    "www.4unitedvoices.com",
  ]
  ...
  snippets = {
    ...
    aetn_services_recv         = { type = "init", priority = 100, content = file("${path.module}/fastly/4unitedvoices-com/aetn_services_recv.vcl") }
    aetn_default_backend       = { type = "init", priority = 100, content = file("${path.module}/fastly/4unitedvoices-com/aetn_default_backend.vcl") }
    redirects                  = { type = "init", priority = 100, content = file("${path.module}/fastly/4unitedvoices-com/redirects.vcl") }
    backends                   = { type = "init", priority = 100, content = file("${path.module}/fastly/4unitedvoices-com/prod-backends.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/4unitedvoices-com/default.vcl")
    }
  }
