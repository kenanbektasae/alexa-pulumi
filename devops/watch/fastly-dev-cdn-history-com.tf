module "dev-cdn-history-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "dev-cdn.history.com"
  ...
  snippets = {
    ...
    aetn_default_backend       = { type = "init", priority = 100, content = file("${path.module}/fastly/cdn-history-com/aetn_default_backend.vcl") }
    aetn_service_cache_fetch   = { type = "init", priority = 100, content = file("${path.module}/fastly/cdn-history-com/aetn_service_cache_fetch.vcl") }
    aetn_services_recv         = { type = "init", priority = 100, content = file("${path.module}/fastly/cdn-history-com/aetn_services_recv.vcl") }
    backend                    = { type = "init", priority = 100, content = file("${path.module}/fastly/cdn-history-com/dev_backend.vcl") }
    redirects                  = { type = "init", priority = 100, content = file("${path.module}/fastly/cdn-history-com/redirects.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/cdn-history-com/default.vcl")
    }
  }
