module "prod-historystore-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "historystore.com"

  domains = [
    "historystore.com",
    "www.historystore.com",
  ]
  ...
  snippets = {
    ...
    aetn_services_recv         = { type = "init", priority = 100, content = file("${path.module}/fastly/historystore-com/aetn_services_recv.vcl") }
    aetn_default_backend       = { type = "init", priority = 100, content = file("${path.module}/fastly/historystore-com/aetn_default_backend.vcl") }
    redirects                  = { type = "init", priority = 100, content = file("${path.module}/fastly/historystore-com/redirects.vcl") }
    backends                   = { type = "init", priority = 100, content = file("${path.module}/fastly/historystore-com/prod-backends.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/historystore-com/default.vcl")
    }
  }
