resource "pagerduty_escalation_policy" "prod-watch-wp-svod-supercronic-escalation-policy" {
  name      = "prod-watch-wp-svod-supercronic-escalation-policy"
  num_loops = 3
  rule {
...
resource "pagerduty_service" "prod-watch-wp-svod-supercronic-service" {
  name              = "prod-watch-wp-svod-supercronic-service"
  escalation_policy = pagerduty_escalation_policy.prod-watch-wp-svod-supercronic-escalation-policy.id
...
resource "pagerduty_service_integration" "prod-watch-wp-svod-supercronic-integration-cloudwatch" {
  name    = data.pagerduty_vendor.cloudwatch.name
  vendor  = data.pagerduty_vendor.cloudwatch.id
  service = pagerduty_service.prod-watch-wp-svod-supercronic-service.id
}

resource "pagerduty_service_integration" "prod-watch-wp-svod-supercronic-integration-datadog" {
  name    = pagerduty_service.prod-watch-wp-svod-supercronic-service.name
  service = pagerduty_service.prod-watch-wp-svod-supercronic-service.id
  vendor  = data.pagerduty_vendor.datadog.id
}

resource "datadog_integration_pagerduty_service_object" "prod-watch-wp-svod-supercronic-service" {
  ...
  service_name = pagerduty_service.prod-watch-wp-svod-supercronic-service.name
  service_key  = pagerduty_service_integration.prod-watch-wp-svod-supercronic-integration-datadog.integration_key
}

resource "aws_cloudformation_stack" "prod-watch-wp-svod-supercronic-alert" {
  provider          = aws.us-east-1
  name              = "prod-watch-wp-svod-supercronic-alert"
  ...
  parameters = {
    Email         = null
    HttpEndpoint  = null
    HttpsEndpoint = "https://events.pagerduty.com/integration/${pagerduty_service_integration.prod-watch-wp-svod-supercronic-integration-cloudwatch.integration_key}/enqueue"
...
resource "aws_cloudformation_stack" "prod-watch-wp-svod-supercronic-ecr" {
  provider          = aws.us-east-1
  name              = "prod-watch-wp-svod-supercronic-ecr"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
  ...
  tags = {
    ...
    "Application ID"             = "WPW"
    Role                         = "infra"
    "aetnd:application"          = "watch-wp-svod-supercronic"
    "aetnd:env"                  = "prod"
    "Line of Business - Primary" = "digital"
    env                          = "prod"
    Environment                  = "prod"
...
resource "aws_cloudformation_stack" "prod-watch-wp-svod-supercronic-service" {
  provider          = aws.us-east-1
  name              = "prod-watch-wp-svod-supercronic-service"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  ...
  parameters = {
    ...
    ParentECRRepositoryStack                       = aws_cloudformation_stack.prod-watch-wp-svod-supercronic-ecr.outputs.StackName
    ParentAlertStack                               = aws_cloudformation_stack.prod-watch-wp-svod-supercronic-alert.outputs.StackName
    ...
    AppCommand                                     = "docker-svod-supercronic-entrypoint"
    AppUser                                        = null
    AppPort                                        = "8080"
    ...
    CPUUtilizationTooHighAlarmEvaluationPeriods    = "60"
...
resource "aws_cloudformation_stack" "prod-watch-wp-svod-supercronic-pipeline" {
  provider          = aws.us-east-1
  name              = "prod-watch-wp-svod-supercronic-pipeline"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
  ...
  parameters = {
    ParentClusterStack          = "prod-watch-cluster"
    ParentServiceStack          = aws_cloudformation_stack.prod-watch-wp-svod-supercronic-service.outputs.StackName
    ParentECRRepositoryStack    = aws_cloudformation_stack.prod-watch-wp-svod-supercronic-ecr.outputs.StackName
