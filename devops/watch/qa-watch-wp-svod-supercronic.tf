resource "pagerduty_escalation_policy" "qa-watch-wp-svod-supercronic-escalation-policy" {
  name      = "qa-watch-wp-svod-supercronic-escalation-policy"
  num_loops = 3
  rule {
...
resource "pagerduty_service" "qa-watch-wp-svod-supercronic-service" {
  name              = "qa-watch-wp-svod-supercronic-service"
  escalation_policy = pagerduty_escalation_policy.qa-watch-wp-svod-supercronic-escalation-policy.id
...
resource "pagerduty_service_integration" "qa-watch-wp-svod-supercronic-integration-cloudwatch" {
  name    = data.pagerduty_vendor.cloudwatch.name
  vendor  = data.pagerduty_vendor.cloudwatch.id
  service = pagerduty_service.qa-watch-wp-svod-supercronic-service.id
}

resource "pagerduty_service_integration" "qa-watch-wp-svod-supercronic-integration-datadog" {
  name    = pagerduty_service.qa-watch-wp-svod-supercronic-service.name
  service = pagerduty_service.qa-watch-wp-svod-supercronic-service.id
  vendor  = data.pagerduty_vendor.datadog.id
}

resource "datadog_integration_pagerduty_service_object" "qa-watch-wp-svod-supercronic-service" {
  ...
  service_name = pagerduty_service.qa-watch-wp-svod-supercronic-service.name
  service_key  = pagerduty_service_integration.qa-watch-wp-svod-supercronic-integration-datadog.integration_key
}

resource "aws_cloudformation_stack" "qa-watch-wp-svod-supercronic-alert" {
  provider          = aws.us-east-1
  name              = "qa-watch-wp-svod-supercronic-alert"
  ...
  parameters = {
    Email         = null
    HttpEndpoint  = null
    HttpsEndpoint = "https://events.pagerduty.com/integration/${pagerduty_service_integration.qa-watch-wp-svod-supercronic-integration-cloudwatch.integration_key}/enqueue"
...
resource "aws_cloudformation_stack" "qa-watch-wp-svod-supercronic-ecr" {
  provider          = aws.us-east-1
  name              = "qa-watch-wp-svod-supercronic-ecr"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
...
resource "aws_cloudformation_stack" "qa-watch-wp-svod-supercronic-service" {
  provider          = aws.us-east-1
  name              = "qa-watch-wp-svod-supercronic-service"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  ...
  parameters = {
    ...
    ParentECRRepositoryStack                       = aws_cloudformation_stack.qa-watch-wp-svod-supercronic-ecr.outputs.StackName
    ParentAlertStack                               = aws_cloudformation_stack.qa-watch-wp-svod-supercronic-alert.outputs.StackName
...
resource "aws_cloudformation_stack" "qa-watch-wp-svod-supercronic-pipeline" {
  provider          = aws.us-east-1
  name              = "qa-watch-wp-svod-supercronic-pipeline"
  capabilities      = ["CAPABILITY_IAM"]
  disable_rollback  = false
  notification_arns = []
  ...
  parameters = {
    ParentClusterStack          = "qa-watch-cluster"
    ParentServiceStack          = aws_cloudformation_stack.qa-watch-wp-svod-supercronic-service.outputs.StackName
    ParentECRRepositoryStack    = aws_cloudformation_stack.qa-watch-wp-svod-supercronic-ecr.outputs.StackName
