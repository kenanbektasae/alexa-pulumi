module "dev-s3-aetv-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "dev-s3.aetv.com"
  ...
  snippets = {
    ...
    backend                    = { type = "init", priority = 100, content = file("${path.module}/fastly/s3-aetv-com/dev_backend.vcl") }
    aetn_services_recv         = { type = "init", priority = 100, content = file("${path.module}/fastly/s3-aetv-com/aetn_services_recv.vcl") }
    aetn_default_backend       = { type = "init", priority = 100, content = file("${path.module}/fastly/s3-aetv-com/aetn_default_backend.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/s3-aetv-com/default.vcl")
    }
  }
