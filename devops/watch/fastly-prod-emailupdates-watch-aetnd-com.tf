module "prod-emailupdates-watch-aetnd-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "emailupdates.watch.aetnd.com"

  domains = [
    "emailupdates.watch.aetnd.com",
    "emailupdates.aetv.com",
    "emailupdates.history.com",
    "emailupdates.mylifetime.com",
  ]
  ...
  snippets = {
    ...
    aetn_default_backend       = { type = "init", priority = 100, content = file("${path.module}/fastly/emailupdates-watch-aetnd-com/aetn_default_backend.vcl") }
    aetn_services_rewrites     = { type = "init", priority = 100, content = file("${path.module}/fastly/emailupdates-watch-aetnd-com/aetn_services_rewrites.vcl") }
    aetn_services_recv         = { type = "init", priority = 100, content = file("${path.module}/fastly/emailupdates-watch-aetnd-com/aetn_services_recv.vcl") }
    backend                    = { type = "init", priority = 100, content = file("${path.module}/fastly/emailupdates-watch-aetnd-com/prod_backend.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/emailupdates-watch-aetnd-com/default.vcl")
    }
  }
