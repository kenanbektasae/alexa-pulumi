module "dev-cdn-watch-aetnd-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "dev-cdn.watch.aetnd.com"

  domains = [
    "dev-cdn.watch.aetnd.com",
  ]
  ...
  snippets = {
    ...
    aetn_services_recv         = { type = "init", priority = 100, content = file("${path.module}/fastly/cdn-watch-aetnd-com/aetn_services_recv.vcl") }
    aetn_default_backend       = { type = "init", priority = 100, content = file("${path.module}/fastly/cdn-watch-aetnd-com/aetn_default_backend.vcl") }
    aetn_services_rewrites     = { type = "init", priority = 100, content = file("${path.module}/fastly/cdn-watch-aetnd-com/aetn_services_rewrites.vcl") }
    aetn_services_proxypass    = { type = "init", priority = 100, content = file("${path.module}/fastly/cdn-watch-aetnd-com/aetn_services_proxypass.vcl") }
    redirects                  = { type = "init", priority = 100, content = file("${path.module}/fastly/cdn-watch-aetnd-com/redirects.vcl") }
    backend                    = { type = "init", priority = 100, content = file("${path.module}/fastly/cdn-watch-aetnd-com/dev_backend.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/cdn-watch-aetnd-com/default.vcl")
    }
  }
