module "prod-4uv-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "4uv.com"
  ...
  snippets = {
    ...
    aetn_services_rewrites     = { type = "init", priority = 100, content = file("${path.module}/fastly/4uv-com/aetn_services_rewrites.vcl") }
    aetn_editorials_proxypass  = { type = "init", priority = 100, content = file("${path.module}/fastly/4uv-com/aetn_editorials_proxypass.vcl") }
    aetn_services_proxypass    = { type = "init", priority = 100, content = file("${path.module}/fastly/4uv-com/aetn_services_proxypass.vcl") }
    aetn_services_recv         = { type = "init", priority = 100, content = file("${path.module}/fastly/4uv-com/aetn_services_recv.vcl") }
    aetn_default_backend       = { type = "init", priority = 100, content = file("${path.module}/fastly/4uv-com/aetn_default_backend.vcl") }
    redirects                  = { type = "init", priority = 100, content = file("${path.module}/fastly/4uv-com/redirects.vcl") }
    backends                   = { type = "init", priority = 100, content = file("${path.module}/fastly/4uv-com/prod-backends.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/4uv-com/default.vcl")
    }
  }
