module "prod-pdk-watch-aetnd-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "pdk.watch.aetnd.com"
  ...
  snippets = {
    ...
    aetn_services_rewrites     = { type = "init", priority = 100, content = file("${path.module}/fastly/pdk-watch-aetnd-com/aetn_services_rewrites.vcl") }
    aetn_services_recv         = { type = "init", priority = 100, content = file("${path.module}/fastly/pdk-watch-aetnd-com/aetn_services_recv.vcl") }
    aetn_default_backend       = { type = "init", priority = 100, content = file("${path.module}/fastly/pdk-watch-aetnd-com/aetn_default_backend.vcl") }
    redirects                  = { type = "init", priority = 100, content = file("${path.module}/fastly/pdk-watch-aetnd-com/redirects.vcl") }
    backends                   = { type = "init", priority = 100, content = file("${path.module}/fastly/pdk-watch-aetnd-com/prod-backends.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/pdk-watch-aetnd-com/default.vcl")
    }
  }
