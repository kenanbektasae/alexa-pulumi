module "prod-alienconstore-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "alienconstore.com"

  domains = [
    "alienconstore.com",
    "www.alienconstore.com",
  ]
  ...
  snippets = {
    ...
    aetn_services_recv         = { type = "init", priority = 100, content = file("${path.module}/fastly/alienconstore-com/aetn_services_recv.vcl") }
    aetn_default_backend       = { type = "init", priority = 100, content = file("${path.module}/fastly/alienconstore-com/aetn_default_backend.vcl") }
    redirects                  = { type = "init", priority = 100, content = file("${path.module}/fastly/alienconstore-com/redirects.vcl") }
    backends                   = { type = "init", priority = 100, content = file("${path.module}/fastly/alienconstore-com/prod-backends.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/alienconstore-com/default.vcl")
    }
  }
