module "dev-configs-watch-aetnd-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "dev-configs.watch.aetnd.com"

  domains = [
    "dev-configs.watch.aetnd.com"
  ]
  ...
  snippets = {
    ...
    aetn_services_recv         = { type = "init", priority = 100, content = file("${path.module}/fastly/configs-watch-aetnd-com/aetn_services_recv.vcl") }
    aetn_default_backend       = { type = "init", priority = 100, content = file("${path.module}/fastly/configs-watch-aetnd-com/aetn_default_backend.vcl") }
    backend                    = { type = "init", priority = 100, content = file("${path.module}/fastly/configs-watch-aetnd-com/dev_backend.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/configs-watch-aetnd-com/default.vcl")
    }
  }
