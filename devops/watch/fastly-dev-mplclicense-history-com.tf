module "dev-mplclicense-history-com-fastly" {
  source = "./modules/fastly/vcl_service"

  service_name = "dev-mplclicense.history.com"

  domains = [
    "dev-mplclicense.history.com",
  ]
  ...
  snippets = {
    ...
    aetn_default_backend       = { type = "init", priority = 100, content = file("${path.module}/fastly/mplclicense-history-com/aetn_default_backend.vcl") }
    aetn_services_proxypass    = { type = "init", priority = 100, content = file("${path.module}/fastly/mplclicense-history-com/aetn_services_proxypass.vcl") }
    aetn_service_cache_fetch   = { type = "init", priority = 100, content = file("${path.module}/fastly/mplclicense-history-com/aetn_service_cache_fetch.vcl") }
    aetn_services_recv         = { type = "init", priority = 100, content = file("${path.module}/fastly/mplclicense-history-com/aetn_services_recv.vcl") }
    aetn_services_rewrites     = { type = "init", priority = 100, content = file("${path.module}/fastly/mplclicense-history-com/aetn_services_rewrites.vcl") }
    backend                    = { type = "init", priority = 100, content = file("${path.module}/fastly/mplclicense-history-com/dev_backend.vcl") }
    redirects                  = { type = "init", priority = 100, content = file("${path.module}/fastly/mplclicense-history-com/redirects.vcl") }
  }

  vcls = {
    main = {
      main    = true
      content = file("${path.module}/fastly/mplclicense-history-com/default.vcl")
    }
  }
